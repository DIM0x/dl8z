...
VPN本来不是用于外网访问，也不是为此设计的，所以它在此方面有比较大的缺点。因为VPN开启后发出的数据能明显看出是被加密过的，所以GFW可以把这些加密的信息通通拦截掉，让你无法连接VPN服务器。同时由于VPN服务器地址是暴露的，如果这个服务器被用于访问外网，很可能遭到封禁。同时由于VPN服务器搭建麻烦等原因，实际上使用VPN协议访问外网的“VPN软件”也越来越少。
...
因此我们可以说，VPN访问外网的方法已经不可靠了，只要国家想，就可以分分钟掐掉所有VPN访问，而专用的代理则更可能幸存。
...
下面就简单介绍一下v2rayN的使用，此处就以v2rayN V6为例，因为V5已经停止更新。
...
此时你只需要复制这串字符，之后点击服务器-从剪贴板导入批量URL即可，或者直接按下ctrl+v快捷键。（你可以只复制一个服务器信息，也可以用换行为分割一次性添加多个服务器）
...
单击一下服务器列表栏，按快捷键ctrl+a，可以全选本页所有服务器（如果你要检测所有保存的服务器，记得把“订阅页面”切换到“所有”）。之后右键选择“测试服务器延迟Tcping”，也就是ctrl+o。软件会自动检测延迟，如果延迟为-1则无法连接到服务器。

此时左键单击表头中的“延迟”一项，列表会自动按照延迟大小排序，这样所有无法连接的服务器就被分开了。

之后我们需要测试服务器到外网之间能否正常连接。