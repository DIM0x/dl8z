...
单击一下服务器列表栏任意处，按快捷键ctrl+a，可以全选本页所有服务器（如果你要检测所有保存的服务器，记得把“订阅页面”切换到“所有”）。之后右键选择“测试服务器延迟Tcping”，也就相当于快捷键ctrl+o。软件会自动检测延迟，如果延迟为-1则意为无法连接到服务器。

此时左键点击表头中的“延迟”一项，列表会自动按照延迟大小排序，这样所有无法连接的服务器就被分开了。

之后我们需要测试服务器到外网之间能否正常连接。软件使用的原理是分别通过每个代理服务器向google发送一个请求，来测算延迟。

通过点选列表中第一个可用的服务器，并按住shift点选列表中最后一个可用的服务器（tcp延迟不为-1的那个），你可以选中中间这所有的服务器，也就是选择了所有可连接上的服务器。之后在选择到的区域里右键，点击测试真连接延迟（ctrl+R），此时状态窗口开始滚动，稍等一段时间之后即可测算出服务器连接到google的延迟。同样，数值为-1代表无法访问外网。
...
如上所述，我们要更改系统或者软件的设置，让它们的数据都流向本地回环IP的对应端口，这样数据就会被代理软件再次发送到代理服务器。而系统代理设置往往是最重要的设置，因为很多软件都会以系统设置为依据来配置自身的代理。

这个系统代理设置就是指

v2rayN提供了一键配置的功能，就是在下方“清除系统代理”的下拉框，若选择“自动配置系统代理”，即可自动更改系统的代理配置，这样系统几乎所有软件都会把数据经过v2rayN软件，传输到代理服务器。选择清除系统代理，则配置系统不使用代理，也就恢复了正常。

但是，不是所有软件都会依照系统代理设置。来更改自己的代理设置。